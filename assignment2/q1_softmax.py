...
def softmax(x):
    ...
    x = x - c  # tf.subtract(x, c)
    x = tf.exp(x)
    x_sum = tf.reduce_sum(x, axis=1, keep_dims=True)
    out = x / x_sum  # tf.div(x, x_sum)
...
def cross_entropy_loss(y, yhat):
    ...
    # 100000 times, 44.2952759266 seconds
    # approach 1
    out = -tf.reduce_sum(tf.to_float(y) * tf.log(yhat))

    # approach 2
    # out = tf.negative(tf.reduce_sum(tf.multiply(tf.to_float(y), tf.log(yhat))))

    # 100000 times, 48.4501740932 seconds
    # approach 3, not efficient, but it used gather_nd. because I think there are many zeros in y.
    # so I tried this approach.
    # y = tf.argmax(y, axis=1, output_type=tf.int32)
    # index = tf.range(0, tf.shape(y)[0], dtype=tf.int32)
    # y = tf.expand_dims(y, axis=1)
    # index = tf.expand_dims(index, axis=1)
    # y_index = tf.concat([index, y], axis=1)
    #
    # yhat_ = tf.gather_nd(yhat, y_index)
    # out = -tf.reduce_sum(tf.log(yhat_))
